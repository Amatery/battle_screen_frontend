...
import { isTeamsLoading, teamsSelector, winnerTeam } from '../../../store/teamSlice/team.selectors'
...
function BattleScreen(): ReactElement {
  ...
  const isLoading = useAppSelector(isTeamsLoading)
        ...
        <div className='battle-screen'>
          {(Object.keys(teams).length > 0) && (
            <div className='teams-container'>
              <Card title={<CardTitle title='Team 1' status={winners === 'Team 1' ? 'Won' : 'Lose'} />}
                    className='team'>
                {teams.firstTeam.map((player: IPlayer) => (
                  <Player key={player._id} {...player} />
                ))}
              </Card>
              <Card title={<CardTitle title='Team 1' status={winners === 'Team 2' ? 'Won' : 'Lose'} />}
                    className='team'>
